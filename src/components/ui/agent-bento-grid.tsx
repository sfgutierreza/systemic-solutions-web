...
export function FeatCard({ title, description, children, className = "" }: FeatCardProps) {
  return (
    ...
    >
      ...
      {/* El visual es una ilustración decorativa: el título y la descripción de
          arriba ya dicen todo. Oculto a lectores de pantalla para no dictar
          "Recepción done 0.2s catálogo 342…", que sería ruido sin significado. */}
      <div
        aria-hidden="true"
        className="relative mt-2 flex-1 w-full rounded-[14px] overflow-hidden border border-border/50 bg-background/50 dark:bg-neutral-950/50"
      >
        {children}
      </div>
    </motion.div>
...
const NODES: NodeConfig[] = [
  { id: 'A', x: 50, y: 120, icon: ChatCircle, label: "MENSAJE", type: 'box' },
  { id: 'Router', x: 125, y: 120, type: 'circle' },
  { id: 'C', x: 200, y: 120, icon: Brain, label: "AGENTE", type: 'box' },
  { id: 'B', x: 280, y: 50, icon: Database, label: "CONTEXTO", type: 'box' },
  { id: 'D', x: 280, y: 190, icon: TerminalWindow, label: "ACCIONES", type: 'box' },
];
...
const NODE_COLORS: Record<string, { bg: string; border: string; text: string; buttonBg: string; buttonBorder: string; buttonText: string }> = {
  A: {
    bg: "bg-teal-500/10 dark:bg-teal-500/5",
    border: "border-teal-500/60 dark:border-teal-400/50",
    text: "text-brand-teal-deep dark:text-teal-400",
    buttonBg: "bg-brand-teal",
    buttonBorder: "border-brand-teal-deep",
  ...
  C: {
    bg: "bg-brand-purple/10 dark:bg-brand-purple/5",
    border: "border-brand-purple/60 dark:border-brand-purple-400/50",
    text: "text-brand-purple-600 dark:text-brand-purple-400",
    buttonBg: "bg-brand-purple",
    buttonBorder: "border-brand-purple-700",
    buttonText: "text-white",
  },
  B: {
    bg: "bg-brand-purple/10 dark:bg-brand-purple/5",
  ...
  return (
    <div className="w-full h-full flex flex-col gap-3.5 justify-between">
      ...
      <div className="flex gap-4 pt-[0.625rem] pr-[0.625rem] pb-0.5 pl-0.5">
        {[
          { label: "Mensajes/día", value: "340", trend: "+8%" },
          { label: "Costo/chat", value: "S/ 0.05", trend: "-3%" },
        ].map((s, i) => {
              ...
              >
                ...
                <div className="flex flex-col min-w-0">
                  ...
                  <div className="flex items-center gap-1.5 mt-2">
                    ...
                    <span className="text-[8px] text-muted-foreground/50 font-mono">ant.</span>
                  </div>
                </div>
...
const STATUS_ICONS: Record<string, { icon: any; color: string; bg: string; gradient: string; border: string }> = {
  done: { icon: Check, color: "text-brand-teal-deep", bg: "bg-teal-500/15", gradient: "bg-gradient-to-b from-teal-400 to-teal-600", border: "border-teal-600" },
  running: { icon: CircleNotch, color: "text-brand-purple-600", bg: "bg-brand-purple/15", gradient: "bg-gradient-to-b from-brand-purple-400 to-brand-purple-600", border: "border-brand-purple-700" },
  waiting: { icon: Clock, color: "text-slate-600", bg: "bg-slate-500/15", gradient: "bg-gradient-to-b from-slate-400 to-slate-600", border: "border-slate-600" },
  idle: { icon: Minus, color: "text-slate-500", bg: "bg-slate-500/10", gradient: "bg-gradient-to-b from-slate-400 to-slate-600", border: "border-slate-600" },
};

export function Card3() {
  const logs = [
    // status = clave interna de color/icono; label = lo que se muestra.
    { agent: "Recepción", action: "Mensaje nuevo por WhatsApp", status: "done", label: "hecho", t: "0.2s" },
    { agent: "Intención", action: "Detecta: quiere agendar una cita", status: "done", label: "hecho", t: "0.9s" },
    { agent: "Agenda", action: "Buscando hueco en Google Calendar…", status: "running", label: "en curso", t: "1.8s" },
    { agent: "Respuesta", action: "Espera confirmación de Agenda", status: "waiting", label: "en espera", t: "—" },
    { agent: "Registro", action: "En cola", status: "idle", label: "inactivo", t: "—" },
  ];
...
const NS_ICONS: Record<string, React.ElementType> = {
  ...
  precios: FileText,
  preguntas: SlackLogo,
  políticas: NotionLogo,
};

const NS_COLORS: Record<string, { bar: string; dot: string; badge: string; buttonBg: string; buttonBorder: string }> = {
  catálogo: { bar: "from-brand-purple to-brand-purple-400", dot: "bg-brand-purple", badge: "bg-brand-purple/15 text-brand-purple-600", buttonBg: "bg-brand-purple", buttonBorder: "border-brand-purple-700" },
  precios: { bar: "from-teal-600 to-teal-400", dot: "bg-brand-teal", badge: "bg-teal-500/15 text-brand-teal-deep", buttonBg: "bg-brand-teal", buttonBorder: "border-brand-teal-deep" },
  preguntas: { bar: "from-slate-600 to-slate-400", dot: "bg-slate-500", badge: "bg-slate-500/15 text-slate-600", buttonBg: "bg-slate-500", buttonBorder: "border-slate-600" },
  políticas: { bar: "from-brand-purple-700 to-brand-purple-400", dot: "bg-brand-purple-700", badge: "bg-brand-purple/15 text-brand-purple-700", buttonBg: "bg-brand-purple-700", buttonBorder: "border-brand-purple-700" },
};

const RETRIEVAL_QUERIES = [
  { ns: "catálogo", q: "stock de zapatillas talla 42", t: "0.2s" },
  { ns: "precios", q: "descuento por 3 unidades", t: "1.1s" },
  { ns: "preguntas", q: "¿hacen delivery los domingos?", t: "2.4s" },
  { ns: "políticas", q: "plazo para cambios y devoluciones", t: "4.0s" },
  { ns: "catálogo", q: "colores disponibles del modelo", t: "5.8s" },
  { ns: "precios", q: "costo de envío a provincia", t: "7.2s" },
];

export function Card4() {
  const namespaces = [
    { name: "catálogo", hits: 342, fill: 88 },
    { name: "precios", hits: 218, fill: 56 },
    { name: "preguntas", hits: 97, fill: 25 },
    { name: "políticas", hits: 54, fill: 14 },
  ];
  ...
  return (
    <div className="w-full h-full flex gap-5 py-2 px-3">
      ...
      <div className="flex-1 flex flex-col gap-0 min-w-0 pr-2">
        <p className="text-[8px] font-mono uppercase tracking-widest text-muted-foreground mb-3">Fuentes</p>
        ...
        <div className="flex items-center gap-2 pt-3 mt-auto">
          ...
          <span className="text-[8px] font-mono text-muted-foreground font-medium tracking-wide">Consultando tus fuentes</span>
        </div>
      </div>
      ...
      <div className="w-[172px] shrink-0 flex flex-col gap-0">
        <p className="text-[8px] font-mono uppercase tracking-widest text-muted-foreground mb-2.5">Consultas</p>
...
export function Card5() {
  const tools = [
    { name: "enviar_whatsapp", calls: 14, icon: Globe, latency: "280ms", color: "bg-gradient-to-b from-teal-400 to-teal-600", borderColor: "border-teal-600" },
    { name: "agendar_cita", calls: 8, icon: TerminalWindow, latency: "1.2s", color: "bg-gradient-to-b from-brand-purple-400 to-brand-purple-600", borderColor: "border-brand-purple-700" },
    { name: "leer_catálogo", calls: 22, icon: FileText, latency: "12ms", color: "bg-gradient-to-b from-slate-400 to-slate-600", borderColor: "border-slate-600" },
    { name: "registrar_pedido", calls: 31, icon: Brain, latency: "95ms", color: "bg-gradient-to-b from-teal-400 to-teal-600", borderColor: "border-teal-600" },
  ];
          ...
          >
            ...
            <div className="flex items-start justify-between">
              ...
              <div className="flex flex-col items-end gap-0.5 mt-0.5">
                ...
                <span className="text-[7px] font-mono text-muted-foreground/80 uppercase tracking-widest leading-none">Usos</span>
              </div>
            </div>
